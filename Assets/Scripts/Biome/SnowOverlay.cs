...
public class SnowOverlay : MonoBehaviour
{
    ...
    void Start()
    {
        // BiomeManager calls GenerateSnow() right after AddComponent; Unity then
        // fires Start() a frame later. Without this guard we build and immediately
        // discard the entire snow mesh set twice per biome.
        if (!_generated) GenerateSnow();
        FixSortingOrder();

    }

    [Header("Full-Map Coverage")]
    [Tooltip("Radius the snow must fill. 0 = auto-detect from the camera at generation. " +
             "Set to your map radius (e.g. 80) for a fixed size. Both the ground disc AND " +
             "the falling-snow box are expanded to this, so nothing stops in a central square.")]
    public float mapCoverageRadius = 0f;

    [Tooltip("Cap on how much element counts may be multiplied when expanding coverage (perf guard).")]
    public float maxCoverageCountMultiplier = 4f;
    ...
    // Reports the REAL extent of disc-placed ground patches (tracked in
    // GetRandomDiscPosition). If maxX/maxY ≈ groundRadius the geometry covers the
    // map and a central "square" is a render/cull/camera issue; if it's small, the
    // generation radius itself is the problem.
    void LogGeneratedExtent()
    {
        if (_extSamples == 0) { Debug.Log("[SnowExtent] No disc patches generated."); return; }
        Vector3 wp = transform.position;
        Debug.Log($"[SnowExtent] disc-patch LOCAL extent x[{_extMinX:F1}..{_extMaxX:F1}] " +
                  $"y[{_extMinY:F1}..{_extMaxY:F1}] samples={_extSamples} | " +
                  $"snowWorldPos={wp} => WORLD x[{_extMinX + wp.x:F1}..{_extMaxX + wp.x:F1}] " +
                  $"y[{_extMinY + wp.y:F1}..{_extMaxY + wp.y:F1}] | " +
                  $"groundMeshes={groundObjects.Count} driftMeshes={driftObjects.Count} " +
                  $"iceMeshes={iceObjects.Count} frostMeshes={frostVegObjects.Count}");
    }

    public void FixSortingOrder()
    {
        // 1. Grab every Mesh (Ground Snow, Drifts, Ice) generated by this script
        MeshRenderer[] meshRenderers = GetComponentsInChildren<MeshRenderer>(true);
        foreach (var mr in meshRenderers)
        {
            mr.sortingLayerName = "Default"; // Change if your background uses a custom layer
            mr.sortingOrder = 0; // Forces it to draw above the background tiles
        }

        // 2. Fix the airborne snow particles so they aren't hidden by tiles
        ParticleSystemRenderer[] psRenderers = GetComponentsInChildren<ParticleSystemRenderer>(true);
        foreach (var psr in psRenderers)
        {
            psr.sortingLayerName = "Default";
            psr.sortingOrder = 50; // Particles should float high above everything
        }
    }
    ...
    // Expand every snow layer to cover the whole visible map, keeping each layer's
    // ORIGINAL density by area-scaling its count. The ground/drift/ice/frost layers
    // are discs (radius); the airborne layer is a BOX (±spawnRadius × ±spawnHeight),
    // which is what produced the off-center "square" of falling snow — making the box
    // as large as the disc removes it. BiomeManager re-creates this component per biome
    // with fresh values, so this runs once per generation and never compounds.
    void ApplyFullMapCoverage()
    {
        float target = mapCoverageRadius;
        if (target <= 0.01f)
        {
            // Auto: cover the largest visible orthographic camera (centered on origin).
            ...
            for (int c = 0; c < cams.Length; c++)
            {
                ...
                float corner = Mathf.Sqrt(cw * cw + ch * ch); // half-diagonal of the view
                target = Mathf.Max(target, corner);
            }
            target *= 1.25f; // headroom so corners/edges are never bare
        }

        // Never shrink below whatever was configured.
        float baseRadius = Mathf.Max(groundRadius, spawnRadius, 0.01f);
        target = Mathf.Max(target, baseRadius);

        // Disc layers: count scales with circle area (target/base)^2.
        float discMult = Mathf.Clamp((target * target) / (baseRadius * baseRadius),
                                     1f, Mathf.Max(1f, maxCoverageCountMultiplier));

        // Airborne box: count scales with its own rectangle-area growth.
        float oldBoxArea = Mathf.Max(0.01f, (2f * spawnRadius) * (2f * spawnHeight));
        float newBoxArea = (2f * target) * (2f * target);
        float boxMult = Mathf.Clamp(newBoxArea / oldBoxArea,
                                    1f, Mathf.Max(1f, maxCoverageCountMultiplier));

        groundRadius = target;   // ground/drift/ice/frost discs reach the map edge
        spawnRadius = target;   // falling-snow box width  = full map
        spawnHeight = target;   // falling-snow box height = full map (square -> gone)

        groundCount = Mathf.RoundToInt(groundCount * discMult);
        driftCount = Mathf.RoundToInt(driftCount * discMult);
        icePatchCount = Mathf.RoundToInt(icePatchCount * discMult);
        frostVegCount = Mathf.RoundToInt(frostVegCount * discMult);
        particleCount = Mathf.RoundToInt(particleCount * boxMult);

        int orthoCams = 0;
        var camList = Camera.allCameras;
        for (int i = 0; i < camList.Length; i++)
            if (camList[i] != null && camList[i].orthographic) orthoCams++;

        Debug.Log($"[SnowCoverage] mapCoverageRadius={mapCoverageRadius:F1} baseRadius={baseRadius:F1} " +
                  $"target={target:F1} => groundRadius={groundRadius:F1} spawnRadius={spawnRadius:F1} " +
                  $"spawnHeight={spawnHeight:F1} | discMult={discMult:F2} boxMult={boxMult:F2} " +
                  $"groundCount={groundCount} particleCount={particleCount} | orthoCams={orthoCams} " +
                  $"snowTransform.worldPos={transform.position} localScale={transform.lossyScale}");
    }
    ...
    Mesh BuildGroundMesh(int count, int seedOffset)
    {
        ...
        // Seed ONCE per mesh chunk, then draw positions from the continuous
        // stream. Calling Random.InitState() per element with consecutive seeds
        // makes the FIRST insideUnitCircle correlate with the seed, biasing every
        // patch to one side of the disc — that was the hard left/right divider.
        Random.InitState(7919 + seedOffset);
    ...
    Mesh BuildDriftMesh(int count, int seedOffset)
    {
        ...
        Random.InitState(31337 + seedOffset); // seed once per chunk (see ground note)
    ...
    Mesh BuildIceMesh(int count, int seedOffset)
    {
        ...
        Random.InitState(54321 + seedOffset); // seed once per chunk (see ground note)
    ...
    Mesh BuildFrostVegMesh(int count, int seedOffset)
    {
        ...
        Random.InitState(99991 + seedOffset); // seed once per chunk (see ground note)
    ...
    void LateUpdate()
    {
        ...
        // issues; isVisible tells us whether the snow mesh is being culled at all.
        if (!_camLogged && groundObjects.Count > 0)
        {
            _camLogged = true;
            var cams = Camera.allCameras;
            string s = $"[SnowCameras] count={cams.Length}";
            for (int i = 0; i < cams.Length; i++)
            {
                var c = cams[i];
                if (c == null) continue;
                s += $"\n   '{c.name}' ortho={c.orthographic} size={c.orthographicSize:F1} " +
                     $"pos={c.transform.position} rect={c.rect} mask=0x{c.cullingMask:X} depth={c.depth} " +
                     $"customCullMatrix={c.useOcclusionCulling}";
            }
            var rend = groundObjects[0].GetComponent<Renderer>();
            if (rend != null)
                s += $"\n   sampleGroundMesh layer={groundObjects[0].layer} isVisible={rend.isVisible} " +
    ...
    Mesh BuildMesh(string name, Vector3[] verts, Color[] cols, Vector2[] uvs, Vector2[] uv2s, int[] tris, int vi, int ti)
    {
        ...
        // Oversize the bounds so an off-center / split-screen camera never
        // frustum-culls these big combined meshes (the cause of the hard line
        // next to the core, where one camera dropped the mesh entirely).
        float coverR = Mathf.Max(Mathf.Max(groundRadius, spawnRadius), 1f);
        mesh.bounds = new Bounds(Vector3.zero, Vector3.one * coverR * 4f);
        return mesh;
    }
    ...
    void OnDestroy()
    {
        // Restore normal frustum culling on every camera we overrode, so the
        // oversized matrix doesn't leak to the next biome / scene.
        var cams = Camera.allCameras;
        for (int i = 0; i < cams.Length; i++)
    ...
    // Actual extent of disc-placed ground patches (ground/drift/ice/frost), for logging.
    private float _extMinX = float.MaxValue, _extMaxX = float.MinValue;
    private float _extMinY = float.MaxValue, _extMaxY = float.MinValue;
    private int _extSamples;
    ...
    Vector2 GetRandomDiscPosition(float radius, float exclusion)
    {
        ...
        if (pos.x < _extMinX) _extMinX = pos.x;
        if (pos.x > _extMaxX) _extMaxX = pos.x;
        if (pos.y < _extMinY) _extMinY = pos.y;
        if (pos.y > _extMaxY) _extMaxY = pos.y;
        _extSamples++;
